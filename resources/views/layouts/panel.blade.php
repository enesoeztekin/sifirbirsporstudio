...
<body class="bg-zinc-800">
    <button id="menu-toggle" class="block lg:hidden focus:outline-none text-white p-5">
    ...
    <div id="menu" class="-translate-x-full fixed duration-200 left-0 lg:translate-x-0 flex flex-col z-20 bg-clip-border bg-amber-500 text-white h-[calc(100vh-2rem)] w-full h-full max-w-[20rem] p-4 shadow-xl shadow-blue-gray-900/5">
        <div class="mb-2 p-4">
          <h5 class="block antialiased tracking-normal font-sans text-2xl font-semibold leading-snug text-white">Yönetim Paneli</h5>
        </div>
        <nav class="lg:flex flex-col gap-5 min-w-[240px] p-2 font-sans text-base font-normal text-white">
          <a href="/dashboard">
      ...
      <main class="lg:ml-[20rem] ml-0 py-8 px-6 text-xl font-bold">
        @yield('content')
      </main>
      @if (Session::has('error') || $errors->any())
            <div class="toast-error fixed bottom-0 w-full bg-red-800 text-white text-center py-3">
              {{Session::get('error')}} {{ $errors->first() }}
            </div>
      @endif

      @if (Session::has('success'))
            <div class="toast-success fixed bottom-0 w-full bg-green-700 text-white text-center py-3">
              {{ Session::get('success') }}
            </div>
      @endif
